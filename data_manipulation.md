...
Use `relocate()` to move variables around but keep everything else the
same.
...
Learning Assessment: in pups data, select litter number, sex, and
pd_ears.
...
Use `filter()` to filter out ROWS. `==` means that is equal to (value
that you want that is equal to this)
...
Learning Assessment: Filter to include only pups with sex 1 and Filter
to include only pups with PD walk less than 11 and sex 2.
...
You can create new variables by manipulating other variables. You can
change the characters data within variables.
